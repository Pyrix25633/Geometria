...
$\mathrm{d}(P,Q)=|\overrightarrow{PQ}|=\sqrt{(x_{Q}-x_{P})^{2}+(y_{Q}-y_{P})^{2}+(z_{Q}-z_{P})^{2}}$

# Tra un punto e un piano
$P=(x_{P},y_{P},z_{P}),\;\pi:ax+by+cz+d=0$
$$\begin{flalign}\mathrm{d}(P,\pi)=\frac{|a\cdot x_{P}+b\cdot y_{P}+c\cdot z_{P}+d|}{\sqrt{a^{2}+b^{2}+c^{2}}} &&\end{flalign}$$
...
$\mathrm{d}(P,r)=\mathrm{d}(P,Q(t_{0}))$
